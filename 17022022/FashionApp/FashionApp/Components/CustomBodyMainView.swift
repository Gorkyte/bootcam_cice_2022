            ...
            ForEach(customData) { item in
                LazyVGrid(columns: Array(repeating: GridItem(), count: 2)) {
                    ForEach(item.myRows){ itemRow in
                        ...
                        NavigationLink{  // ===============Para navegar dentro de la foto(Botón plano)
                                        // ===============Unido al "navigationlink" del padre Fashion View
                            DetailFashionView(model: itemRow)
                        } label: {
                            CardCell(model: itemRow)
                        }
                        .buttonStyle(PlainButtonStyle())// ===============Para navegar dentro de la foto (Botón plano)
...
var customData = [CustomType (id:0,
                              myRows: [Row(id: 0, name: "Fit and Flare", image: "p1", price: "€ 199"),
                                       ...
                                       Row(id: 3, name: "Fit and Vives", image: "p22", price: "€ 499"),
