...
final class NetworkService: NetworkServiceProtocol {
    ...
    let defaultHTTPHeaders: HTTPHeaders = {
        let BearerAuthentification = "Bearer 12345678"
        return[Utils.Constantes().Authentication: Utils.Constantes().BearerAuthentification]
    } ()
    ...
    func requestGeneric<M>(requestPayload: RequestDTO,
        ...
        do {
            let jasonObject = try JSONDecoder().decode(entityClass.self, from: dataUnw)
