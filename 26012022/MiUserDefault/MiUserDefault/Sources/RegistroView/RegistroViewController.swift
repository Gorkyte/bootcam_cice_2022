...
class RegistroViewController: UIViewController {
    ...
    @IBAction func guardarDatosACTION(_ sender: Any) {
        if self.datosCumplimentados() {
            ...
            let imagenData = self.ImagenPerfil.image?.jpegData(compressionQuality: 0.5)
            Utils.Constants().kPrefs.setValue(imagenData, forKey: Utils.Constants().kImage)
            
            let fechaActualizacion = Date()
            Utils.Constants().kPrefs.setValue(fechaActualizacion, forKey: Utils.Constants().kFecchaActualizacion)
            
            self.present(Utils.shared.muestraAlerta(titulo: "Saludos a todos",
                                                    mensaje: "Estamos salvando los datos con éxito",
                                                    completionHandler: { _ in
                                                        self.ususarioLogado = true
                                                        Utils.Constants().kPrefs.setValue(self.ususarioLogado, forKey: Utils.Constants().kUserLogado)
            ...
            self.present(Utils.shared.muestraAlerta(titulo: "Debes cumplimentar el formulatior",
                                                    mensaje: "Adelante selecciona una foto y te registrarás con éxcito",
    ...
    private func mostrarDatosActualizados() {
        ...
        self.fechaActualizacionLBL.text = "Ultima actualizacion: \(dateFormatter.string(from: Utils.Constants().kPrefs.object(forKey: Utils.Constants().kFecchaActualizacion) as! Date))"
